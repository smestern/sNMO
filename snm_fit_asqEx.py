...
from b2_model.asqEx import adExModel
...
_cm_range = [5, 350] #in pF
_taum_range = [9, 250] #in ms
_EL_range = [-90, -50] #inmV
_VT_range = [-70., -20.] #in mV
_tauw_range = [0.01, 550.] #in ms
_a_range= [-2,5] #in ns
_b_range = [0.00001,100] #in pA
_VR_range = [-80., -40] #in mV
_deltaT_range = [0.5, 5] #in mV
_bVT_range = [-5, 5] #in mV
_tauVT_range = [1, 500] #in ms
vars = [_cm_range,
        _taum_range,
        _EL_range,
        _a_range, 
...
param_dict = {'VT': var_range[0], 'tauw': var_range[1], 'a':var_range[2], 'b':var_range[3], 'VR': var_range[5], }
_labels = ['Cm', 'taum', 'EL', 'a', 
                    'tauw', 'VT', 'VR',
                    'b', 'DeltaT', 'bVT', 'tauVT']

# === MAIN FUNCTION ===

def run_optimizer(file, optimizer='ng', rounds_=500, batch_size_=500, sweep_upper_cut=None):
    ...
    if optimizer == 'skopt' or optimizer=='ng':
        vars_stack = np.vstack(vars)
        results = _opt(model, vars_stack, _labels, optimizer=optimizer, id=cell_id)
    elif optimizer == 'snpe':
...
def vars_to_param_space():
    '''
    Transforms the vars defined as a high low pair into the prior distro expected for SNPE
    '''
...
def plot_trace(param_dict, model):
    ...
    for x in [*model.subthresholdSweep, model.spikeSweep[-2]]:
        ...
        if len(spikes.t) > 0:
            scatter(spikes.t/second, np.full(spikes.t.shape[0], 20) ,label="Sim spike times", marker='x', zorder=9999)
    ylim(-150, 50)
...
def load_data_and_model(file, sweep_upper_cut=None):
    ...
    Cm = Cm/pF
    taum *=1000
    vars[0] = [Cm*0.90, (1.1*Cm)]
    vars[1] = [taum * 0.75, (1.25*taum)]
    vars[2] = [compute_el*1.01, compute_el*0.99]
    if vars[0][0] < 0:
        vars[0][0] = 0.1
    if vars[1][0] < 0:
        vars[1][0] = 0.1
    model = adExModel({'EL': compute_el, 'dt':dt, '_run_time':2, 'Cm': Cm, 'taum': taum})
...
def SNPE_OPT(model, id='nan', use_post=False, refit_post=False, run_ng=True, run_ng_phase=False, run_skopt=False):
    ...
    if use_post:
        ...
        if os.path.exists(f"{id}_post.pkl"):
            with open(f"{id}_post.pkl", "rb") as f:
                posterior = load(f)
        elif os.path.exists(f"post.pkl"):
            ...
            posterior = inference(num_simulations=N*num_sim)
            with open(f"{id}_post.pkl", "wb") as f:
                dump(posterior, f)
    elif refit_post:
        #if we are going to refit still load the post
        with open("post.pkl", "rb") as f:
            posterior = load(f)
    else:
        prior = vars_to_param_space() #Converts our ranges into the format used by SBI
    ...
    posterior_samples = posterior.sample((5000,), x=x_o) #sample 5,000 points
...
def _opt(model, var_pairs, _labels, optimizer='ng', id='nan'):
        '''
        ...
        #add in refrac as a term
        var_pairs = np.vstack((var_pairs, [0,10]))
        _labels.append('refractory')



        opt = snmOptimizer(var_pairs, _labels, batch_size, rounds, backend=optimizer, nevergrad_opt=ng.optimizers.ParaPortfolio)#
        ...
        for i in np.arange(rounds):
            ...
            _, error_t, error_fi, error_s = model.opt_full_mse(param_dict)
