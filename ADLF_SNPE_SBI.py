...
n=14
file_path = file_dir +'//..//NWB_with_stim//macaque//V1//M19_JS_A1_C14.nwb'
realX, realY, realC, _ = loadNWB(file_path, old=False)
realX, realY, realC = realX[:n,:], realY[:n,:], realC[:n,:] #Take only the first 14 sweeps
...
rm = membrane_resistance_subt(realX[non_spiking_sweeps,:], realY[non_spiking_sweeps,:], realC[non_spiking_sweeps,:]) * Gohm
cm = ((tau) / rm)
cm_scaled = cm/pF
rm = (rm * ohm) / Gohm
tau *=1000
...
## from https://elifesciences.org/articles/56261j simulator, 
...
if load_infer == True:
    ...
    theta, x = simulate_for_sbi(simulator, proposal=prior, num_simulations=int(N*batches), num_workers=1, simulation_batch_size=N)
...
def loss_mse(param_set):
    ...
    mse = np.nan_to_num(mse, nan=9999, posinf=999, neginf=9999)
    return mse.tolist()

# Just use SKOPT for now
N=50
import skopt
opt = skopt.Optimizer(var_pairs, base_estimator='RF', n_initial_points=50, n_jobs=-1)
...
for x in [0, 3, 8]:
    voltage, _ = adif_model(*res.x, realC_i=realC[x,:], record_v=True)
    plt.plot(realX[x, :], realY[x,:], c='k')
    plt.plot(voltage[0].t/second, voltage[0].v/mV, c="g")
    voltage2, _ = adif_model(*params, realC_i=realC[x,:], record_v=True)
    plt.plot(voltage[0].t/second, voltage2[0].v/mV, c="r")
plt.show()
